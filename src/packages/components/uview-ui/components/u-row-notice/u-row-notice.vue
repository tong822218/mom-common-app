<template>
	<view class="u-notice" @tap="clickHandler" :style="{ '--leftPadding': paddingLeft }">
		<slot name="icon">
		...
		<view class="u-notice__content" ref="u-notice__content">
			<text ref="u-notice__content__text" class="u-notice__content__text" :style="[textStyle]">
				<text class="u-notice-text">{{ text }}</text>
			</text>
		</view>
		<view class="u-notice__right-icon" v-if="['link', 'closable', 'more'].includes(mode)">
	...
	export default {
		...
		methods: {
			...
			async vue() {
				...
				fontText = (await this.$uGetRect(".u-notice-text")).width;

				if (this.rollLive) {
					if (fontText < boxWidth) {
						this.paddingLeft = "0%";
						return;
					} else {
						this.paddingLeft = "50%";
