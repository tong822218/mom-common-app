<template>
  <view class="signature-wrap">
    <view class="img-wrap" @click="touchSignature()">
      <!--       @touchstart.stop="touchSignature()"
      
      @top="showSignature()"
       -->
      <slot :img="absPrevView">
        <image :src="absPrevView" mode="scaleToFill"></image>
        <!-- <view v-else>
          <view>
            <image
              class="icon"
              src="@/images/qianzi.png"
              mode="scaleToFill"
            ></image
          ></view>
    ...
    <view v-if="!disabled" v-show="show" class="signature-contain">
      <view class="signature-main" style="z-index: 3000;">
        <view class="signature-title"
          ><text :key="i" v-for="(t, i) in titles">{{ t }}</text></view
        >
        <canvas
          disable-scroll="true"
          class="signature"
...
export default {
  ...
  methods: {
    ...
    getImagePath(data) {
      ...
      // console.log(333, URL.createObjectURL(this.dataURItoBlob(data)));
      // let png = this.convertCanvasToImage();
      this.$emit("getImage", data);
      // base64ToPath(data)
      //   .then((path) => {
      //     console.log(path);
      //     this.$emit("getImage", path);
      //   })
      //   .catch((error) => {
      //     console.error(error);
      //   });
